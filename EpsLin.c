...
//  v1.64:
//       - Added/fixed support for Raspberry Pi
//  v1.63:
//       - Some small fixes for prints to allow better integration
//         to Ensoniq-Remote
//       - Fix spelling errors
//
//  v1.62:
...
//
//  v1.45: [from Neo] (forked by Thomas Arashikage)
...
//         line arguments (for ex. Cygwin requires always parameter for -p)
...
#ifndef __ARM_ARCH
#include <sys/io.h>   // For Raspberry Pi, sys/io.h is not available as it merged to other headers.
#endif
    ...
    for (j = 0; j < 9; j++)
    {
      ...
      else
      {
        if (j == 8)
        {
          ...
          printf("    \"ff\":%d,\n", data[1]);
          printf("    \"media_id\":%d,\n", data[2]);
          printf("    \"disk_label\":\"%c%c%c%c%c%c%c\",\n",
                 data[3],
                 data[5],
                 data[7],
                 data[9],
...
int GetEFEs(char media_type, FD_HANDLE fd, int in, unsigned char EFE[MAX_NUM_OF_DIR_ENTRIES][EFE_SIZE], char *process_EFE, unsigned char *DiskFAT, int printmode)
{
  ...
  for (j = 0; j < MAX_NUM_OF_DIR_ENTRIES; j++)
  {
    ...
    if ((type > 9) && (type < 23))
      continue; // skip SD-1/VFX-SD entries, however implausible...
    if (type > 34)
    ...
    int printmode)
{
  ...
  PutEFE(process_EFE, 1, EFE, media_type, image_type,
         ...
         fd, DiskFAT, DiskHdr, MemDataHdr, MemData, printmode);
...
int main(int argc, char **argv)
{
  ...
  case PUT: // Put EFEs
    ...
    PutEFE(process_EFE, start_idx, EFE, media_type, image_type,
           ...
           dir_start, dir_cont, total_blks, &free_blks, fat_blks,
           fd, DiskFAT, DiskHdr, NULL, NULL, printmode);
    break;
  ...
  case MKDIR: // Make Dir
    MkDir(process_EFE, EFE, media_type, image_type,
          ...
          &free_blks, fat_blks,
          fd, DiskFAT, DiskHdr, mkdir_name, printmode);
    break;
