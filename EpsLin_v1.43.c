/////////////////////////////////////////////////////
//
// EpsLin v1.43 (c) Juha Forsten 9 Dec / 2018
// -------------------------------------------------
...
void GetMedia(char *arg, int argc, char *media_type, char *image_type,
  ...
  if ((arg == NULL) || ((IsEFE(arg) == OK) && (argc >= 3))) {
    ...
    *media_type = 'f';
    ...
    // Check if conversion is needed!
    if ((*image_type != EPS_TYPE) && (*image_type != ASR_TYPE) &&
        (*image_type != OTHER_TYPE)) {
...
int main(int argc, char **argv)
{
  ...
  mode = NONE; subdir_cnt = 0; image_type= -1; printmode = HUMAN_READABLE;
  trk_size =  0; media_type  = 0; fat_blks   = 0; in = 0; 
  DiskFAT = NULL; start_idx = 1;
  confirm_operation = 0;
  ...
  while (1) {
    ...
    case 'C': // ** CHECK MEDIA **
      ...
      GetMedia(argv[optind], argc, &media_type, &image_type, &nsect, &trk_size,
               &fd, in_file, &in);
      if (optarg == NULL)
        check_level = 0;
      else
